...
chatlink = telebot.types.InlineKeyboardButton(text='76:;:№(;(;(_ Чат воркеров', url=config.chatlink)
...
db = sqlite3.connect(config.dirbot+'database.db', check_same_thread=False)
sql = db.cursor()
...
if sql.fetchone() is None:
	...
	sql.execute(f"INSERT OR IGNORE INTO infoteam VALUES (?, ?, ?)", (0, 0, dt_now))
	db.commit()
...
def send_welcome(message):
	...
	if str(sql.fetchall()) == '[]':
		...
		if stwork == 999:
			...
			bot.send_sticker(message.chat.id,'5066903451:AAErw8Y58fpqUf5SNFDSGYGU1w6ISB_t6t0', reply_markup=buttons_start)
